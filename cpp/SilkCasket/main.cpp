...
Java_eternal_future_efmodloader_utility_SilkCasket_release(JNIEnv *env, jobject /* this */, jstring temp, jstring path, jstring target) {
...
extern "C"
JNIEXPORT void JNICALL
Java_eternal_future_efmodloader_utility_SilkCasket_compress(JNIEnv *env, jobject /* this */, jstring temp, jstring path, jstring target) {
    const char *tempStr = env->GetStringUTFChars(temp, nullptr);
    const char *pathStr = env->GetStringUTFChars(path, nullptr);
    const char *targetStr = env->GetStringUTFChars(target, nullptr);
